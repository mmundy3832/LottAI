"""
prepare.py - Data loading and feature engineering for Pick 3 Evening autoresearch.
...
Data: Pick 3 Evening (3,893 draws, Sept 9 2013 - Feb 13 2026)
...
_DATA_FILE = os.path.join(os.path.dirname(__file__), "..", "pick3evening.csv")
...
_TRAIN_END = 2725       # first 70%: rows 0..2724  (2725 rows)
_VAL_END = 3310         # next 15%:  rows 2725..3309 (585 rows)
#                         final 15%: rows 3310..3892 (583 rows)
...
def _load_raw():
    ...
    df = df[["date", "d1", "d2", "d3", "combo", "combo_int",
...
def load_data():
    """Return the full Pick 3 Evening DataFrame (cached)."""
    global _DF_CACHE
...
_CACHE_DIR = os.path.join(os.path.dirname(__file__), ".feature_cache")

def precompute_all_features():
    """Pre-compute all feature sets for train and val indices, save to .npy files.

    Only recomputes if cache is missing. Delete .feature_cache/ to force rebuild.
    """
...
def _print_summary():
    ...
    print("=" * 65)
    print("  Pick 3 Evening - Autoresearch Data Module")
    print("=" * 65)
