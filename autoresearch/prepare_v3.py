"""
prepare_v3.py - Data loading and feature engineering for Pick 3 All Draw Times + Equipment.

Drop-in replacement for prepare.py and prepare_v2.py.
Loads the combined dataset (all draw times) including equipment features
(machine ID and ball set IDs).

Data: Pick 3 All Draw Times (15,572 draws)
Columns: date, draw_time, machine_used, ballset1_used, ballset2_used, ballset3_used, d1, d2, d3
...
_TRAIN_END = 10900   # rows 0..10899
_VAL_END   = 13236   # rows 10900..13235, test is 13236..15571
...
def _load_raw():
    """Load the raw CSV and return a clean DataFrame."""
    df = pd.read_csv(
        _DATA_FILE,
        dtype=str,
    )
    ...
    # draw_time as int (0=morning, 1=day, 2=evening, 3=night)
    ...
    # Keep columns we need, in order
    df = df[["date", "draw_time", "machine_used", "ballset1_used",
             "ballset2_used", "ballset3_used",
             "d1", "d2", "d3", "combo", "combo_int",
...
def load_data():
    """Return the full Pick 3 combined DataFrame (cached)."""
    global _DF_CACHE
...
# ---------------------------------------------------------------------------
# Equipment Wear/Age Stats (built at module load from full dataset)
# ---------------------------------------------------------------------------
# _EQUIP_STATS[i] gives 8 wear/age features for row i, computed using only
# rows prior to i (no data leakage).

_EQUIP_STATS = None  # shape (N, 8), computed once at module load


def _build_equip_stats():
    """Build equipment wear/age stats from the full dataset (vectorized)."""
    df = load_data()  # full 15572-row df, sorted by (date, draw_time)
    n = len(df)
    stats = np.zeros((n, 8), dtype=np.float64)

    # machine_total_uses: number of prior rows with same machine_used
    stats[:, 0] = df.groupby('machine_used').cumcount().values

    # machine_days_age: days from first appearance of this machine to this row's date
    first_machine_date = df.groupby('machine_used')['date'].transform('min')
    stats[:, 1] = (df['date'] - first_machine_date).dt.days.values

    # ball set features, one set per position
    for i, col in enumerate(['ballset1_used', 'ballset2_used', 'ballset3_used']):
        offset = 2 + i * 2
        stats[:, offset]     = df.groupby(col).cumcount().values
        first_bs_date        = df.groupby(col)['date'].transform('min')
        stats[:, offset + 1] = (df['date'] - first_bs_date).dt.days.values
...
def _feat_draw_time(df, idx):
    """One-hot encoding of draw_time column (4 features).

    0=morning, 1=day, 2=evening, 3=night
    """
    dt = int(df.iloc[idx]["draw_time"])
    oh = np.zeros(4, dtype=np.float64)
    oh[dt] = 1.0
    return oh


def _feat_equipment(df, idx):
    """Equipment wear/age features (8 features).

    Features (all count only draws BEFORE this one -- no leakage):
      0: machine_total_uses   - cumulative draws on this machine prior to this draw
      1: machine_days_age     - calendar days from machine's first appearance to this draw
      2: ballset1_total_uses  - cumulative uses of this ball set in position 1
      3: ballset1_days_age    - calendar days from ball set's first use in position 1
      4: ballset2_total_uses  - same for position 2
      5: ballset2_days_age    - same for position 2
      6: ballset3_total_uses  - same for position 3
      7: ballset3_days_age    - same for position 3
    """
    # Fast path: use precomputed stats for the standard training df
    if _EQUIP_STATS is not None and idx < len(_EQUIP_STATS):
        return _EQUIP_STATS[idx].copy()

    # Slow path: live eval uses extended df (full_df + live_rows appended)
    # Compute on-the-fly for this row using only prior rows (no leakage)
    row = df.iloc[idx]
    past = df.iloc[:idx]
    result = np.zeros(8, dtype=np.float64)

    # machine features
    machine = row['machine_used']
    machine_past = past[past['machine_used'] == machine]
    result[0] = len(machine_past)
    if len(machine_past) > 0:
        result[1] = (row['date'] - machine_past['date'].min()).days

    # ball set features
    for i, col in enumerate(['ballset1_used', 'ballset2_used', 'ballset3_used']):
        bs = row[col]
        bs_past = past[past[col] == bs]
        offset = 2 + i * 2
        result[offset] = len(bs_past)
        if len(bs_past) > 0:
            result[offset + 1] = (row['date'] - bs_past['date'].min()).days
...
FEATURE_SETS = {
    ...
    "draw_time": _feat_draw_time,
    "equipment": _feat_equipment,
}
...
_CACHE_DIR = os.path.join(os.path.dirname(__file__), ".feature_cache_v3")

def precompute_all_features():
    """Pre-compute all feature sets for train, val, and test indices, save to .npy files.

    Only recomputes if cache is missing. Delete .feature_cache_v3/ to force rebuild.
    """
...
def _print_summary():
    ...
    print("=" * 65)
    print("  Pick 3 All Draw Times + Equipment - Autoresearch Data Module")
    print("=" * 65)
    ...
    print(f"  Machines:          {df['machine_used'].nunique()} unique")
    print(f"  Ball sets:         {pd.concat([df['ballset1_used'], df['ballset2_used'], df['ballset3_used']]).nunique()} unique")
    print(f"  -" * 32)
    print(f"  Train set:  {len(train):>6,} draws  "
          f"({train['date'].iloc[0].strftime('%Y-%m-%d')} to "
          f"{train['date'].iloc[-1].strftime('%Y-%m-%d')})")
    print(f"  Val set:    {len(val):>6,} draws  "
          f"({val['date'].iloc[0].strftime('%Y-%m-%d')} to "
          f"{val['date'].iloc[-1].strftime('%Y-%m-%d')})")
    print(f"  Test set:   {len(test):>6,} draws  "
...
# Build equipment wear/age stats at module load time (uses full dataset)
_EQUIP_STATS = _build_equip_stats()
