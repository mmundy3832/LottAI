"""
live_eval.py -- Evaluate ALL (or top-N) experiments against unseen live draw data.

The live data is pick3evening_live.csv: draws from Feb 14 2026 onward, never
seen during any training, validation, or GA optimization run. This is a true
out-of-sample test -- the GA has no knowledge of these draws.

Writes results to live_eval_results.jsonl and prints a leaderboard sorted by
live_optimal_ev (the metric that matters for actual deployment).

Usage:
    python -u live_eval.py                 # all successful experiments
    python -u live_eval.py --n 200         # top 200 by val optimal_ev
    python -u live_eval.py --dry-run       # show count, exit
    python -u live_eval.py --resume        # skip already-evaluated experiment IDs
"""
...
_OUT_JSONL  = os.path.join(_DIR, "live_eval_results.jsonl")
_LOG_FILE   = os.path.join(_DIR, "live_eval.log")
_TMP_FILE   = os.path.join(_DIR, "experiment_live_eval.py")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all experiments on live out-of-sample data")
    parser.add_argument("--n",        type=int, default=None,
                        help="Limit to top-N by val optimal_ev (default: all)")
    parser.add_argument("--resume",   action="store_true",
                        help="Skip experiment IDs already in live_eval_results.jsonl")
    parser.add_argument("--dry-run",  action="store_true",
                        help="Count candidates and exit without running")
    args = parser.parse_args()

    log_msg(f"\n{'='*60}")
    log_msg(f"live_eval.py  --n={args.n}  --resume={args.resume}")
    log_msg(f"Live data: {_LIVE_PATH}")
    log_msg(f"{'='*60}")

    if not os.path.exists(_LIVE_PATH):
        log_msg(f"ERROR: live data file not found: {_LIVE_PATH}")
        sys.exit(1)
